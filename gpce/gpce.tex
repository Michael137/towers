...
optimal as it is not user-most interpreter.
\AMcomment{I tried to re-write next sentence but still unclear...}
While a definitional interpreter can be staged using TDPE by simply lifting
its return values, the process of staging an abstract machine requires a more sophisticated binding-time analysis with
({\it i}) careful design of its division rules
and ({\it ii}) consideration\AMcomment{??} of which locations
\AMcomment{variables? expressions? SECD locations?}
...
tailored to the SECD stack-registers
\AMcomment{there's only one stack register}
