    ...
    \centering
        \begin{tikzpicture}
                        ...
                        ]
            {
              ...
              \draw (m-1-1.south west) |- (m-4-1.north east) |- (m-1-1.south west);
              \draw (m-4-1.south west) |- (m-6-1.south east) |- (m-4-1.south west);
              \draw (m-4-1.south west) |- (m-8-1.south east) |- (m-6-1.south west);
              \draw (m-4-1.south west) |- (m-9-1.south east) |- (m-8-1.south west);
              \draw (m-9-1.south west) -- (m-10-1.south) -- (m-9-1.south east);
        \end{tikzpicture}
...
\begin{figure}[htp!]
    ...
    \begin{subfigure}[htp!]{\linewidth}
        \centering
            ...
            \end{tikzpicture}
            \caption{A tombstone diagram representation of our experimental tower where \mevl{} is our toy language described in section \ref{subsec:mevl}, \mslang{} refers to the multi-level language introduced as part of Pink. Lisp* is Pink's Lisp based front-end \cite{amin2017collapsing}. \textit{JVM} is the Java Virtual Machine \cite{lindholm2014java} and in our diagram also encompasses any underlying machinery necessary to run the JVM.}
            \label{fig:tombstone}
    \end{subfigure}\\[4ex]
    \begin{subfigure}[htp!]{\linewidth}
        \centering
            ...
            {
             Program   &        &       &       &      &                                            \\[2ex]
               Python  & Python &  $\to$ &  x86 ASM & x86 ASM   &                                   \\
                       &        &    C  &                                                           \\[3ex]
                       &        &    &       &      x86          &                                  \\
                       &        &    &       &      Emulator     &                                  \\[3ex]
                       &        &    &       &      JavaScript   &                                  \\
            ...
            \draw (m-2-2.south west) |- (m-2-4.north east) |- (m-3-3.north east) |- (m-3-3.south west) |- (m-2-2.south west);
            \draw (m-2-4.south east) |- (m-5-5.south east) |- (m-2-4.north east);
            \draw (m-5-5.south east) |- (m-6-5.south west) |- (m-5-5.south east);
            \draw (m-6-5.south west) -- (m-7-5.south) -- (m-6-5.south east);
            \end{tikzpicture}
...
The state transitions for the \textbf{LIFT} operation in the staged SECD machine are shown in \eqref{eq:secd_sem_lift}. The intended use of the instruction is to signal \mslang{} to lift the top of the stack. We do this by dispatching to the builtin \textit{lift} operator provided by \secdlisp. Thus, \mint{lisp}|   LDC 10 LIFT STOP| would generate a \mslang{} code expression representing the constant 10,
...
%TODO: \cite{danvy1996pragmatics} "Reflect is applied to types occurring negatively in the source type.". No the case with Pink?
