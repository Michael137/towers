...
We now provide a redesign to the internal \texttt{RAP} calling convention while keeping the semantics of the instruction in-tact in order to allow SECD style recursive function calls without the need for mutable variables. The idea is to create wrap the recursive SECD instructions in a closure at the LISP-level, performing residualization on the closure and distinguishing between returning from a regular as opposed to recursive function to ensure termination of the specializer.

%TODO explain above paragraph further
\begin{figure}[ht]
\centering
\begin{alignat}{5}
		\text{v.s e (} \mathbf{RTN}\text{.c) (s{\ts} e{\ts} c\ts  d\ts  fns\ts.d) fns} & \longrightarrow \:
		 ...
		 & && \text{(lift-all v)}								\qquad\;\;\text{otherwise} \nonumber \\
		\nonumber \\%
