...
(if (eq? 'LIFT (car ops))
 (let r
	...
	(if (lambda? (car s))
		...
		(let bt (cdr (cddr mem))
		(let rec (lift (lambda rec x ((((((machine '()) 'ret) (cons (cons rec (cdr s1)) fns)) bt) (caar s1)) (cons (cons x '()) (cdr (cdr c)))))) rec)))))))
	(let _ _ ;(log 0 (cons 'GOTHERE (car s)))
...
(if (eq? 'LDR (car ops))
  ((((((machine (cons (((locate (car (cadr ops))) (cadr (cadr ops))) fns) s)) (cons 'fromldr d)) fns) bt) (cddr ops)) env)
(if (eq? 'LDF (car ops))
  ((((((machine (cons
...
(if (eq? 'RTN (car ops))
  (if (and (pair? d)
    ...
    (if (code? (maybe-lift 1)) ;this means "RTN" was called immediately after "LDR"
      (let _ _ ;(debug 'FROMLDR)
      (mla (let s1 (car s)
        (cons
          (lambda _ arg ((car s1) (mla (cons (car arg) (cddr s1)))))
          (cdr s1)))))
      (let _ _ ;(debug 'FROMLDRNORMAL)
      (car s)))
  (if (eq? 'ret d)
    (let _ _ ;(debug 'RET)
    (mla (car s)))
    (let _ _ ;(debug 'RTNELSE)
...
(if (eq? 'RAP (car ops))
  ...
  (let mem (cons newDump (cons recOpsEnvEnv (cons fns bt)))
  (let rec (maybe-lift (lambda rec env ((((((machine '()) 'ret) (cons (cons (cons rec (cons mem recEnv)) oldEnv) fns)) bt) recOps) env)))
