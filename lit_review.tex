...
\subsection{Normalization By Evaluation (NBE)}
...
In \cite{danvy1999type} Danvy builts a language in ML that supports reification and reflection (section 1) and then shows how partial evaluation of can be achieved using a ``normalization function'':
...
\begin{alignat*}{3}
		\text{RTN:} &																																							\\
		& \text{v.s e (RET.c) (s{\ts} e{\ts} c\ts  d\ts  fns\ts.d) fns} \longrightarrow &&							\\
