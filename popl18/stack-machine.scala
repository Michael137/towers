...
object Stack_Machine {
  ...
  (let stack '()
    (let exec (lambda exec stk (lambda _ op
                                  (if (eq? (maybe-lift 'PUSH) (car op)) (maybe-lift (cons stk (cadr op)))
                                  (if (eq? (maybe-lift 'POP) (car op)) (maybe-lift (cdr stk))
                                  (if (eq? (maybe-lift 'ADD) (car op)) (maybe-lift (cons (cddr stk) (+ (car stk) 
                                                                                                       (car (cdr stk)))
                                                                                  )
                                                                      )
                                      (maybe-lift stk)
