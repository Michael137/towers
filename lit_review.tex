...
\begin{enumerate*}[label=(\arabic*)]
	...
	%TODO: introduce and discuss the problem of propagating BTA information through a tower of interpreters
	%TODO: show difficulties of using TDPE on abstract machines
\end{enumerate*}.
...
Instead we can use an $\eta$-expansion to turn instances of $f$ as a higher-order value into static function applications. The resulting annotations are shown in \eqref{eq:tdpe_ex3a} and the $\eta$-expansion is highlighted in green. After static reduction we obtain the optimal expression in \eqref{eq:tdpe_ex3b} that only contains dynamic values, a single unfolding of $f$ and $\beta$-redexes. Danvy then generalizes the $eta$-expansion and introduces a general class of coercions that permit residualization of static values in dynamic contexts. We represent such coercions with a $\downarrow^t$ which represents the conversion from a static value of type $t$ to a dynamic value. Using the coercion operator, TDPE would yield the annotation in \eqref{eq:tdpe_ex4}.
...
To summarize
TDPE: residualization that performs BTA based on types, static reduction is normalization, need coercion operators to yield optimal code, define reify (which we call lift to be consistent with Pink)/reflect shown in figure \ref{fig:tdpe_rules} from \cite{danvy1999type}, residulazation amounts to reification of a term and then static reduction, implementation used in \mslang{} follows the table and uses a methodology attributed to Eijiro Sumii in \cite{danvy1999type}
...
%TODO: bracket highlighting inconsistent
\begin{figure}[ht]
\centering
...
methodology
    built own experimental tower based on hypothetical model
    staged SECD machine
    created mevl
    staged mevl
...
%TODO: noise is due to interpretation overhead
...
leaking implementations between layers, base language getting bloated with features
