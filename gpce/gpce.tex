...
Our experiments are availbale from \textit{**omitted for double-blind review**}.
...
requires adapting the following other instructions:
\begin{itemize}
\item LDF which loads a function, now requires tagging
\item AP which applies, now requires deciding whether to apply or generate code for application (to avoid infinite unrolling)
\item RTN which returns, now requires deciding whether to continue with the dump (default) or return a value (to satisfy calling convention of lower level)
\end{itemize}
These changes cross-cut the original design.
