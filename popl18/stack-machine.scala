...
object Stack_Machine {
  ...
  */
  ...
  val machine_src = s"(let maybe-lift (lambda _  e e) $stack_machine_src)"
  val machinec_src = s"(let maybe-lift (lambda _  e (lift e)) $stack_machine_src)"

  def test() = {
    println("// ------- Stack_Machine.test --------")

    checkrun(s"(($machine_src '(_ * a _ * done)) '(b a done))", "Str(yes)")
    checkrun(s"(($machine_src '(_ * a _ * done)) '(b b done))", "Str(no)")
    checkrun(s"((run 0 ($machinec_src '(_ * a _ * done))) '(b a done))", "Str(yes)")
    checkrun(s"((run 0 ($machinec_src '(_ * a _ * done))) '(b b done))", "Str(no)")

    checkcode(s"($machinec_src '(a b done))",
    """(lambda f0 x1 
  (let x2 (car x1) 
  (let x3 (eq? 'done x2) 
  (if x3 'no 
  ...
  (if x5 'no 
    ...
    checkrunlog(s"((($instr_src '$machine_src) '(a b done)) '(a b done))", "Str(yes)", logres)
    checkrunlog(s"((run 0 (($instr2_src '$machinec_src) '(a b done))) '(a b done))", "Str(yes)", logres)
    checkrunlog(s"((run 0 ((run 0 ($instrc_src '$machinec_src)) '(a b done))) '(a b done))", "Str(yes)", logres)
    checkcode(s"(($instr2_src '$machinec_src) '(a b done))", log_ab_code)
    checkcode(s"((run 0 ($instrc_src '$machinec_src)) '(a b done))", log_ab_code)
