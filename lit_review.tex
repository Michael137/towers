...
%TODO move FNS to after sec:secd_staged
\begin{table}[!htbp]
  \centering
...
(let machine (lambda _ stack (lambda _ dump (lambda _ control (lambda _ environment
    ...
    (let initStack '()
    (let initDump '()
        (lambda _ ops (((((machine initStack) initDump) ops))))
\end{minted}
...
%TODO: left align
	...
	\begin{equation*}
		\text{s e (} \mathbf{LDR} \text{ (i j).c) d fns} \longrightarrow \text{(($\lambda$.(locate i j fns)).s) e c d fns}
	\end{equation*}
