...
(lambda _ maybe-lift (lambda tie eval (lambda _ exp (lambda _ env
  ...
  (if (sym?           (car exp))
    ...
    (if (eq?  '>      (car exp))   (> ((eval (cadr exp)) env) ((eval (caddr exp)) env))
    (if (eq?  'and    (car exp))   (and ((eval (cadr exp)) env) ((eval (caddr exp)) env))
    (if (eq?  'or     (car exp))   (or ((eval (cadr exp)) env) ((eval (caddr exp)) env))
    ...
    (if (eq?  'lambda? (car exp))  (lambda? ((eval (cadr exp)) env))
